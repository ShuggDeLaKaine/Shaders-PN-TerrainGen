...
{
	glm::vec3 position = glm::vec3(x, y, 0.001f);
	double perlinNoise = cycleOctaves(position, 10);

	//xyz positions.
	vertices->push_back((float)x);		//xPos
	vertices->push_back(perlinNoise);			//yPos - always 0 for now. Going to calculate this on GPU - can change to calclaue it here.
...
double Terrain::cycleOctaves(glm::vec3 position, int numOctaves)
{
	float total = 0.0f;
	...
	{
		double x = position.x * frequency;
		double y = position.y * frequency;

		total += perlin.noise(x, y, 0.01f) * amplitude;
